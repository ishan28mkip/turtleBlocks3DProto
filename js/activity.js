...
define(function(require) {
    ...
    require(['domReady!'], function(doc) {
        ...
        function init() {      
            ...
            trashContainer = new createjs.Container(); //Done
            turtleContainer = new createjs.Container();
            stage.addChild(trashContainer, blocksContainer,
                           palettesContainer);
            // Other file reference start
            // This binds the motion functions to the easel js container, this is not required to be implemented right now in the 3D version
            setupBlocksContainerEvents();

            //Reference to the objects in the individual files.
            trashcan = new Trashcan(canvas, trashContainer, cellSize, refreshCanvas); //done
            turtles = new Turtles(canvas, turtleContainer, refreshCanvas); //done
            blocks = new Blocks(canvas, blocksContainer, refreshCanvas, trashcan, stage.update); //done, not required in the current version
            ...
            if (URL.indexOf('?') > 0) {
                ...
                if (urlParts[1].indexOf('=') > 0) {
                    projectName = urlParts[1].split('=')[1];
                }
            }
            if (projectName !== null) {
